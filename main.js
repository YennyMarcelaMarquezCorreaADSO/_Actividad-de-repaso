addEventListener("DOMContentLoaded", (e)=>{
    ...
    myform.addEventListener("submit", (e)=>{
        ...
        for(let id in data[dataInput.scale]){
            if(data[dataInput.scale][id][0] <= dataInput.nota && data[dataInput.scale][id][1] >= dataInput.nota){
                ventane.showModal();
                let myTabla = document.querySelector("tbody");
                myTabla.innerHTML = "";
                myTabla.insertAdjacentHTML("beforeend",`
                    <tr>
                        <td>${data["Traditional"][id].join("-")}</td>
                        <td>${data["Point_Range"][id].join("-")}</td>
                        <td>${data["Letter"][id]}</td>
                        <td>${data["SBG_Rating"][id]}</td>
                        <td>${data["Proficiency_level_witch_standard"][id]}</td>
                    </tr>
                    <tr>
                        <td colspan="5">${(id == 0) ? "Querido usuario se gano un viaje a <b>Cancún</b>" : (id == 1) ? "Querido usuario se gano unas voletas para Cine Colombia para ver la pelicula de <b>Black Adam</b>" : ""}</td>
                    </tr>
                `);
            }
        }
